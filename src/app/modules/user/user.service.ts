...
import { walletValidationSchema } from "../wallet/wallet.validation";
...
const updateUserIntoDB = async (id: string, payload: Partial<TUser>) => {
  const { wallets, billing_information, personal_information, contact_information, ...remainingStudentData } = payload;
...
const addNewWallet = async (id: string, payload: TUserWallet) => {
  try {
    const createdData = await User.findByIdAndUpdate(id,
...
const updateWallet = async (id: string, wallet_id: string, payload: TUserWallet) => {
