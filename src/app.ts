...
app.use(
  cors({
    origin: ['https://mining.robofxtrader.com','http://localhost:3001','https://cryptominerx.com','http://localhost:3000'],
    credentials: true,
  }),
);
...
cron.schedule(
  ...
  async () => {
    ...
    for (const rigData of aggregatedData) {
      ...
      const user = await User.findById(userid, { status: 'approved' })
        .select('profit')
        .select('balance')
        .select('grossBalance');
      ...
      const totalActiveTime = 86400 - totalInactiveTime;
